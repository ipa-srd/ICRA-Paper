...
This leads to the approach of using a mixture density network \cite{bishop1994mixture}. The network learns from parameters $\pi$, $\mu$, $\sigma$ and $\rho$ according to \cite{graves2013generating} a distribution is given as:
\begin{equation}
...
As the future movement depends on previous positions, the parameters $\mu$, $\sigma$, $\rho$ and $\pi$ are derived from a LSTM cell, which takes positions as input. The network is trained unsupervised and therefore optimizes itself given time.